...
function renderMarkdown(text: string): React.ReactNode[] {
  ...
  for (const line of lines) {
    if (line.startsWith('### ')) {
      flushList();
      result.push(
        <h2 key={k()} className="text-2xl font-serif text-natural-text mt-10 mb-2">
          {line.slice(4)}
        </h2>
      );
    } else if (line.trim() === '---') {
...
// ─────────────────────────────────────────────
export default function MedicinaDelLavoroPage() {
  // ── SEO ──────────────────────────────────────
  useSEO({
      ...
      <section className="pt-32 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <h1 className="text-4xl lg:text-5xl font-serif text-natural-text mb-6 leading-tight">
              {service.title}
            </h1>
          ...
          >
              ...
              <div className="mb-12">
                ...
                <div className="space-y-4">
                  {service.faq.map((faq, i) => (
                    <div key={i} className="bg-white p-6 rounded-2xl border border-natural-border">
                      <h3 className="font-bold text-natural-text mb-2 text-sm">{faq.question}</h3>
            ...
            <div className="bg-natural-accent p-8 rounded-[32px] text-white">
              <h3 className="text-2xl font-serif mb-4">Richiedi questo servizio</h3>
              <p className="text-white/80 text-sm mb-8 leading-relaxed">
                Il nostro staff è a tua disposizione per un'analisi delle necessità della tua
                azienda e per fissare un appuntamento.
              </p>
              <div className="flex flex-col gap-3">
                <a
                  href={CONTACT.whatsappMessage('Medicina del Lavoro')}
                  target="_blank"
